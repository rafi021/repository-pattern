...
class MakeRepository extends Command
{
    ...
    {
        $name = str_replace(config("repository-pattern.repository_suffix"), "", $this->argument("name"));
    ...
    {
        $repositoryInterfaceNamespace = config("repository-pattern.repository_namespace") . "\Interfaces";
        $repositoryInterfaceName = $className . config("repository-pattern.repository_interface_suffix");
        $stubProperties = [
    ...
    private function createRepository(string $className, $isDefault = true)
    {
        $repositoryNamespace = $isDefault
            ? config("repository-pattern.repository_namespace") . "\\" . $this->getDefaultImplementation()
            : config("repository-pattern.repository_namespace");

        $repositoryName = $className . config("repository-pattern.repository_suffix");
        $stubProperties = [
    ...
    {
        return $this->appPath() . "/" .
            config("repository-pattern.repository_directory") .
            "/Interfaces/$className" . config("repository-pattern.repository_interface_suffix") . ".php";
    }
    ...
    private function getRepositoryPath($className, $isDefault)
    {
        $path = $isDefault
            ...
            : "/Other/$className" .  config("repository-pattern.repository_suffix") . ".php";

        return $this->appPath() . "/" .
            config("repository-pattern.repository_directory") . $path;
    }
    ...
    {
        $this->ensureDirectoryExists(config("repository-pattern.repository_directory"));
        $this->ensureDirectoryExists(config("repository-pattern.repository_directory") . "/Interfaces");
        $this->ensureDirectoryExists(config("repository-pattern.repository_directory") . "/" . $this->getDefaultImplementation());
    }
    ...
    {
        return config("repository-pattern.default_repository_implementation");
    }
}
